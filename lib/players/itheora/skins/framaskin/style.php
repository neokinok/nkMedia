...
.msg_download, .msg_options, .msg_info { /* Texte dans video */
	...
	text-align: justify;
	overflow: auto;
	background : url(<?php echo $spath; ?>player/background.jpg) no-repeat #000000;
	z-index :3;
}
...
.top {
	position: absolute; 
	left: 10px;
	width: <?php echo $wvid ;?>px; height: 30px;
...
<?php   // largeur minimale pour l'affichage de l'infobulle 
	...
	top: 10px;
	left: 70px;
	width: <?php echo $wvid - 120;?>px; height: 16px;
	text-align:center;
	color: #000000;
}
...
.stop a, .play a, .playlist a, .torrent a, .download a, .options a, .share a {
	position: absolute;
	top:9px;
	width: 18px;
	height : 18px;
...
/* Correctif couleur par rapport a defaut pour Framaskin */
a:link, a:visited, p, h1, .playlist_title, h1 a, table, .info,
.msg_playlist .msg, .msg_error .msg, .msg_share .msg, 
.msg_download .msg, .msg_options .msg, .msg_info .msg, 
.msg_share table, .msg_download table, .msg_options table, .msg_error table  {color : #6b6bb7; }


.msg_playlist, .msg_error, .msg_share, 
.msg_download, .msg_options, .msg_info { background-color : #ffffff; }
.codebox { background-color: #c4c4ff; }
